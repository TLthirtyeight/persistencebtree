...
public class PersistenceBTreePane extends Pane {
	...
	public void drawBTree(BTNode<String> root, double x, double y) {
		if (root != null) {
			...
			for (int currentLevel = level - 1; currentLevel >= 0; currentLevel--) {
				int recttangleY = currentLevel * levelVerticalDist;
	...
	private void drawLines() {
		...
		while (iterator.hasNext()) {
			...
			if (child.getFather() != null) {
				...
				int startX1 = parentLocation[0] + parent.getSize() * rectangleWidth / 2;
	...
	private void markDownNodeLocation(BTNode<String> btNode, int rectangleX, int recttangleY) {
		nodeLocationMap.put(btNode, new int[]{rectangleX, recttangleY});
	}
